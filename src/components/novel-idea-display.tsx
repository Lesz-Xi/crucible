...
import { StatisticalMetricsPanel } from "./statistical-metrics-panel";
...
interface NovelIdeaCardProps {
  ...
  isSelected?: boolean;
  onSelect?: () => void;
  pdfSourceUrls?: Record<string, string>;
}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {bridgedConcepts.slice(0, 4).map((concept, i) => (
          <span
            key={i}
        ...
        {bridgedConcepts.length > 4 && (
          <span className="px-2 py-1 text-xs text-gray-500">
            +{bridgedConcepts.length - 4} more
          </span>
        )}
        {bridgedConcepts.length === 0 && (
          <span className="px-2 py-1 text-xs text-gray-500">No bridged concepts available</span>
        )}
      </div>
      ...
      {criticalAnalysis && (
        <div className="mb-4 p-4 bg-red-900/10 border border-red-500/20 rounded-xl">
           <div className="flex items-center justify-between mb-2">
             ...
             <span className="text-xs font-mono text-red-400">Validity: {criticalAnalysis.validityScore ?? "N/A"}/100</span>
           </div>
           
           <p className="text-sm text-gray-400 mb-2 italic">"{criticalAnalysis.critique ?? "No critique provided."}"</p>
           
           {(logicalFallacies.length > 0 || biasDetected.length > 0) && (
             <div className="flex flex-wrap gap-2 mt-2">
               {logicalFallacies.map((f, i) => (
                 <span key={`f-${i}`} className="px-1.5 py-0.5 text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 rounded">
                   Fallacy: {f}
                 </span>
               ))}
               {biasDetected.map((b, i) => (
      ...
      {idea.statisticalMetrics && (
        <StatisticalMetricsPanel metrics={idea.statisticalMetrics} />
      )}

      {idea.evidenceSnippets && idea.evidenceSnippets.length > 0 && (
        <div className="mb-4 p-3 bg-gray-900/40 rounded-xl border border-gray-700/50">
          <h4 className="text-xs font-semibold text-gray-300 uppercase tracking-wider mb-2">
            Evidence Links
          </h4>
          <div className="space-y-2">
            {idea.evidenceSnippets.map((evidence, i) => (
              <CitationLink
                key={`${evidence.snippet}-${i}`}
                snippet={evidence.snippet}
                page={evidence.page}
                fileName={evidence.sourceName || "source.pdf"}
                pdfUrl={evidence.sourceName ? pdfSourceUrls[evidence.sourceName] : undefined}
              />
            ))}
          </div>
        </div>

...
x
## =
y to denote the counterfactual sentence
...
and
setting the value of X to a constant x will yield a
...
x, y�
,, given that x and y did in fact occur.
Note that lower case letters (e.g., x, y) stand for propo­
sitions (or events). Note also the abbreviations
## Yx
for
## Y
x
= true and y� for Y
x
= false.  Readers accustomed
to writing "A > B" for the counterfactual "B if it were
## �
A" can translate Eq. (2) to read PN =
P(x' > y'lx,y).
...
disease and exposure did in fact occur." This counterfactual notion is also used frequently in lawsuits, where legal responsibility is at the center of contention (see Section 4).
...
fected by x, the antecedent of the counterfactual
## Yx
## ,
...
knowledge about causal effects P(y
x
)  and  P(
## Yx'
)  is
also available
## 1
. Moreover, under some general assump­
...
distribution on the four  binary variables: X, Y,
## Y
x
and Y
x'
, specifying the sixteen parameters of this dis­
...
by
manually enumerating the vertices as described in
These bounds are guaranteed to be sharp because the optimization is global.
...
## 592
## UNCERTAINTY
## IN ARTIFICIAL INTELLIGENCE PROCEEDINGS 2000
sation
## :
## 0
## P(y
...
perimental conditions x or x
## '
is independent of the ac­
tual value of X.
...
Combining Eq. (19) with the constraints of
## (6)-(8),
the
...
[Bailey et al.
## ,
1994]. Eq. (23) shows that this practice
...
## The
condition of  exogeneity, as  defined in  Eq.
## (19)
is testable by comparing experimental and nonexperi­
mental data. A stronger version of exogeneity can be
defined as the joint independence {Y
x,
## Y
x'
} llX which
was
called "strong ignorability" by  Rosenbaum and
...
of
Eqs. {20}-{22), and, moreover, PN,  PS, and PNS
...
sharper bounds on the causal effects P(y
x
) and P(
## Yx
## '
...
## P(y)
## (29)
Compared with Eq. (13), the lower bound for
## P(
## Yx
## )
and the upper bound for P(
## Yx')
are tightened. The
...
information about the causal effects P(y
x)
and P(
## Yx'
## )
...
monotonicity)
## If
X is exogenous and Y is monotonic relative to X,
...
PN, is given by the excess-risk-ratio (ERR):
$$ PN = \frac{P(y \mid x) - P(y \mid x')}{P(y \mid x)} = 1 - \frac{1}{RR} \quad (36) $$
...
x
## '
). We  will call the r.h.s. of (38) by
...
two assumptions can be ascertained
## :
exogeneity (i.e.,
...
drug x.
Moreover, argues the  plaintiff, Mr.  A  is
...
deaths (in thousands) among drug users (x
## )
and non­
users (x').
## Experimental
...
of deaths among the control group
## P(
## Yx
## ')
would have
...
be P(y
x
) = 0.016, eight times higher than 0.002.
...
conditions X
## =
x and we wish to predict whether an
...
on
the first draft. Full version of  this paper will ap­
...
M.  Green. Reference guide on  epidemiol-
ogy.
...
## Semantics,
Computation, and  Applications. PhD
...
Uncertainty in  Artificial Intelligence, pages 202-
- Morgan Kaufmann, San Francisco, CA, 1998.
...
## New York, 1996.
[Khoury et al., 1989] M.J.  Khoury, W.D.  Flanders,
...
## 2000.
[Robins and Greenland, 1989] J .M. Robins and

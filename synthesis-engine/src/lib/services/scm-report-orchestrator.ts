...
export class SCMReportOrchestrator {
  ...
  async generateReport(
    ...
    options: {
      k?: number;
      maxSourcesForExtraction?: number;
    ...
    const k = Math.min(Math.max(1, options.k ?? 6), 8);
    const maxSourcesForExtraction = Math.min(
      Math.max(1, options.maxSourcesForExtraction ?? 8),
      8
    );
    ...
    try {
      ...
      const extractionSources = scoredSources.slice(0, maxSourcesForExtraction);

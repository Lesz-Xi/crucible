...
const DEFAULT_LLM_CONFIG: LLMConfig = {
  provider: 'anthropic',
  model: 'claude-sonnet-4-6', // Updated to latest default
  temperature: 0.7,
};
      ...
      try {
        ...
        if (user?.id && persistenceRef.current) {
          ...
          if (result.success && result.data) {
            // Un-stick any pending operations from previous unclosed sessions
            const cleanedData = result.data.map(exp => 
              exp.status === 'pending' 
                ? { ...exp, status: 'failure' as const, error_message: 'Execution aborted (Session reset)' } 
                : exp
            );
            dispatch({ type: "SET_EXPERIMENT_HISTORY", payload: cleanedData });
            
            // Background cleanup in persistence
            cleanedData.filter(e => e.status === 'failure' && e.error_message === 'Execution aborted (Session reset)').forEach(exp => {
                persistenceRef.current?.updateExperiment(exp.id, { status: 'failure', error_message: 'Execution aborted (Session reset)' }).catch(() => {});
            });
          }
        }

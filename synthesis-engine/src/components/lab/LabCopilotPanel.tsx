...
export function LabCopilotPanel({ isOpen, onToggle }: LabCopilotPanelProps) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        onClick={onToggle}
        className={cn(
          "flex h-full w-10 flex-col items-center justify-center gap-2",
          "border-l border-[var(--lab-border)] bg-[var(--lab-panel-soft)]",
          "text-[var(--lab-text-tertiary)] hover:bg-[var(--lab-hover-bg)] hover:text-[var(--lab-text-primary)] transition-colors"
        )}
  ...
  return (
    <motion.div 
      ...
      className="fixed lg:sticky lg:top-0 right-0 z-[60] flex h-[100dvh] lg:h-screen w-full lg:w-[400px] shrink-0 flex-col border-l border-white/10 dark:bg-black/95 bg-zinc-900/95 shadow-2xl backdrop-blur-3xl"
    >
      ...
      <div className="flex items-center gap-2 border-b border-[var(--lab-border)] bg-[var(--lab-panel)] px-3 py-2.5">
        ...
        <span className="flex-1 text-xs font-semibold uppercase tracking-wider text-[var(--lab-text-primary)]">
          Lab Copilot
        </span>
          ...
          {MODES.map((m) => (
            <button
              ...
              className={cn(
                ...
                mode === m.id
                  ? "bg-[var(--lab-accent-rust)] text-[var(--lab-panel)]"
        ...
        <button
          ...
          className="text-[var(--lab-text-tertiary)] hover:text-[var(--lab-text-primary)] transition-colors"
          title="Settings"
        >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4 min-h-0">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-8 space-y-4">
            <motion.div 
              animate={{ scale: [1, 1.05, 1], boxShadow: ["0 0 0 0 rgba(6,182,212,0)", "0 0 20px 5px rgba(6,182,212,0.3)", "0 0 0 0 rgba(6,182,212,0)"] }} 
              transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} 
              className="relative flex items-center justify-center w-20 h-20 rounded-[2rem] bg-gradient-to-tr from-cyan-950 to-cyan-500/20 border border-cyan-500/30 shadow-2xl"
            >
            ...
            <div className="space-y-1">
              <p className="text-base font-semibold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-emerald-400">
                Synthesis Lab Copilot
              </p>
              <p className="max-w-[240px] text-xs text-zinc-400 leading-relaxed">
                {state.currentStructure
      ...
      <div className="px-3 pb-3 pt-1">
        <div className="flex items-end gap-2 rounded-xl border border-[var(--lab-input-border)] bg-[var(--lab-input-bg)] px-3 py-2 transition-colors focus-within:border-[var(--lab-border-strong)]">
          <textarea
            ref={textareaRef}
            value={prompt}
            ...
            rows={2}
            className="flex-1 resize-none bg-transparent text-sm leading-relaxed text-[var(--lab-text-primary)] placeholder:text-[var(--lab-text-tertiary)] focus:outline-none"
          />
          <button
            type="button"
            ...
            className={cn(
              ...
              prompt.trim() && !isSubmitting
                ? "bg-[var(--lab-accent-rust)] text-[var(--lab-panel)] hover:brightness-105"
                : "cursor-not-allowed text-[var(--lab-text-tertiary)]/50"
            )}
          >
            {isSubmitting ? (
        ...
        <p className="mt-1 text-center text-[10px] text-[var(--lab-text-tertiary)]">
          ↵ Send · Shift+↵ New line
        </p>
      </div>
    </motion.div>
